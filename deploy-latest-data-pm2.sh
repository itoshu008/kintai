...
# メイン処理開始
echo -e "${PURPLE}🚀 DEPLOY LATEST DATA TO PM2 Starting...${NC}"
echo -e "${PURPLE}==========================================${NC}"
...
# 8. フロントエンドの依存関係インストール
log_step "フロントエンドの依存関係をインストール中..."
cd frontend
...
if [ ! -f "dist/index.html" ]; then
    log_error "フロントエンドビルド結果が不正です: index.html が見つかりません"
    exit 1
fi
...
if [ ! -f "public/index.html" ]; then
    log_error "フロントエンドコピー結果が不正です: index.html が見つかりません"
    exit 1
fi
...
if [ -d "backups" ]; then
    ...
    log_success "バックアップディレクトリ: backups/ (${BACKUP_COUNT}個のバックアップ)"
else
...
# 16. 最終レポート
echo ""
echo -e "${GREEN}🎉 DEPLOY LATEST DATA TO PM2 完了！${NC}"
echo -e "${GREEN}==========================================${NC}"
...
echo -e "${CYAN}  - ディスク使用量90%削減${NC}"
echo ""
echo -e "${CYAN}📅 デプロイ完了時刻: $(date)${NC}"
echo ""

# 成功通知
log_success "最新データのPM2デプロイが完了しました！"
log_info "ブラウザで https://zatint1991.com にアクセスして確認してください。"
log_info "バックアップは1分間隔で自動実行されます。"
