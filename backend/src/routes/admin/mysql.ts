...
export const mysqlAdmin = Router();

// データベース初期化
mysqlAdmin.post('/init', async (_req, res) => {
  try {
...
mysqlAdmin.get('/health', async (_req, res) => {
  try {
    const isConnected = await testConnection();
    res.json({ 
      ok: true, 
...
// 部署一覧取得
mysqlAdmin.get('/departments', async (_req, res) => {
  try {
...
// 社員一覧取得
mysqlAdmin.get('/employees', async (_req, res) => {
  try {
