...
export default function MasterPage() {
  ...
  const loadEmployeeMonthlyData = useCallback(async (employeeCode: string, month: string) => {
    ...
    try {
      ...
      const promises = dates.map(dateStr => api.master(dateStr).catch(() => ({ list: [] })));
      const results = await Promise.all(promises);
      
      const allData = results.flatMap(r => r.list || []);
      const filteredDetails = allData
        .filter(row => row.code === employeeCode)
        .map((row, index) => ({ ...row, date: dates[index] })); // 日付情報を付与
        
      setEmployeeDetails(filteredDetails);
    } catch (e: any) {
      setMsg(`❌ 月別データ取得エラー: ${e.message}`);
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  return (
    <div style={{ padding: '24px', background: '#f0f2f5', minHeight: '100vh', fontFamily: 'sans-serif' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '300px 1fr', gap: '24px' }}>
        ...
        {/* 左カラム: 社員一覧 */}
        <div style={{ background: 'white', borderRadius: '8px', padding: '16px', boxShadow: '0 1px 4px rgba(0,0,0,0.08)', height: 'fit-content' }}>
          <h3 style={{ marginTop: 0, marginBottom: '16px' }}>社員一覧</h3>
          <div style={{ maxHeight: '60vh', overflowY: 'auto' }}>
            {sorted.map(r => (
              <div key={r.id} onClick={() => setSelectedEmployee(r)} style={{ padding: '12px', cursor: 'pointer', borderRadius: '6px', marginBottom: '8px', background: selectedEmployee?.code === r.code ? '#e6f7ff' : '#fafafa', border: `1px solid ${selectedEmployee?.code === r.code ? '#91d5ff' : '#eee'}` }}>
                {r.name} ({r.code})
              </div>
            ))}
            {sorted.length === 0 && <p style={{ color: '#888', textAlign: 'center' }}>表示する社員がいません</p>}
          </div>
        </div>
