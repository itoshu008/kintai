...
# 3. ポート確認
Write-Host "🔌 ポート8000の使用状況:" -ForegroundColor Cyan
try {
    $portCheck = netstat -an | Select-String ":8000"
    if ($portCheck) {
        Write-Host "✅ ポート8000でリスニング中" -ForegroundColor Green
        Write-Host $portCheck
    } else {
        Write-Host "❌ ポート8000でリスニングしていません" -ForegroundColor Red
    }
} catch {
...
# ローカルテスト
Write-Host "ローカル (http://localhost:8000/api/admin/health):" -ForegroundColor Yellow
try {
    $response = Invoke-WebRequest -Uri "http://localhost:8000/api/admin/health" -UseBasicParsing -TimeoutSec 5
