...
# メイン処理開始
echo -e "${PURPLE}🚀 ULTIMATE POWER DEPLOY Starting...${NC}"
echo -e "${PURPLE}================================${NC}"
...
if ! npm install --prefer-offline --no-audit --silent; then
    handle_error "フロントエンド依存関係のインストールに失敗しました"
fi
...
if ! npm install --prefer-offline --no-audit --silent; then
    handle_error "バックエンド依存関係のインストールに失敗しました"
fi
...
# 8. 最終レポート
echo ""
echo -e "${GREEN}🎉 ULTIMATE POWER DEPLOY 完了！${NC}"
echo -e "${GREEN}================================${NC}"
