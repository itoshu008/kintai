...
# メイン処理開始
echo -e "${PURPLE}🚀 DEPLOY BACKUP SYSTEM Starting...${NC}"
echo -e "${PURPLE}====================================${NC}"
...
# 6. バックアップディレクトリの準備
log_step "バックアップディレクトリを準備中..."
BACKUP_DIR="/home/zatint1991-hvt55/zatint1991.com/backups"
mkdir -p "$BACKUP_DIR"
log_success "バックアップディレクトリ準備完了: $BACKUP_DIR"
...
if [ -d "$BACKUP_DIR" ]; then
    ...
    log_success "バックアップディレクトリ: $BACKUP_DIR (${BACKUP_COUNT}個のバックアップ)"
...
# 13. 最終レポート
echo ""
echo -e "${GREEN}🎉 DEPLOY BACKUP SYSTEM 完了！${NC}"
echo -e "${GREEN}====================================${NC}"
...
echo -e "${CYAN}  - バックアップディレクトリ: $BACKUP_DIR${NC}"
echo ""
...
# 成功通知
log_success "バックアップシステムのデプロイが完了しました！"
